...
function brush(g){g.each(function(){var g=d3.select(this).style("pointer-events","all").style("-webkit-tap-highlight-color","rgba(0,0,0,0)").on("mousedown.brush",brushstart).on("touchstart.brush",brushstart);var background=g.selectAll(".background").data([0]);background.enter().append("rect").attr("class","background").style("visibility","hidden").style("cursor","crosshair");g.selectAll(".extent").data([0]).enter().append("rect").attr("class","extent").style("cursor","move");var resize=g.selectAll(".resize").data(resizes,d3_identity);resize.exit().remove();resize.enter().append("g").attr("class",function(d){return"resize "+d}).style("cursor",function(d){return d3_svg_brushCursor[d]}).append("rect").attr("x",function(d){return/[ew]$/.test(d)?-3:null}).attr("y",function(d){return/^[ns]/.test(d)?-3:null}).attr("width",6).attr("height",6).style("visibility","hidden");resize.style("display",brush.empty()?"none":null);var gUpdate=d3.transition(g),backgroundUpdate=d3.transition(background),range;if(x){range=d3_scaleRange(x);backgroundUpdate.attr("x",range[0]).attr("width",range[1]-range[0]);redrawX(gUpdate)}if(y){range=d3_scaleRange(y);backgroundUpdate.attr("y",range[0]).attr("height",range[1]-range[0]);redrawY(gUpdate)}redraw(gUpdate)})}brush.event=function(g){g.each(function(){var event_=event.of(this,arguments),extent1={x:xExtent,y:yExtent,i:xExtentDomain,j:yExtentDomain},extent0=this.__chart__||extent1;this.__chart__=extent1;if(d3_transitionInheritId){d3.select(this).transition().each("start.brush",function(){xExtentDomain=extent0.i;yExtentDomain=extent0.j;xExtent=extent0.x;yExtent=extent0.y;event_({type:"brushstart"})}).tween("brush:brush",function(){var xi=d3_interpolateArray(xExtent,extent1.x),yi=d3_interpolateArray(yExtent,extent1.y);xExtentDomain=yExtentDomain=null;return function(t){xExtent=extent1.x=xi(t);yExtent=extent1.y=yi(t);event_({type:"brush",mode:"resize"})}}).each("end.brush",function(){xExtentDomain=extent1.i;yExtentDomain=extent1.j;event_({type:"brush",mode:"resize"});event_({type:"brushend"})})}else{event_({type:"brushstart"});event_({type:"brush",mode:"resize"});event_({type:"brushend"})}})};function redraw(g){g.selectAll(".resize").attr("transform",function(d){return"translate("+xExtent[+/e$/.test(d)]+","+yExtent[+/^s/.test(d)]+")"})}function redrawX(g){g.select(".extent").attr("x",xExtent[0]);g.selectAll(".extent,.n>rect,.s>rect").attr("width",xExtent[1]-xExtent[0])}function redrawY(g){g.select(".extent").attr("y",yExtent[0]);g.selectAll(".extent,.e>rect,.w>rect").attr("height",yExtent[1]-yExtent[0])}function brushstart(){var target=this,eventTarget=d3.select(d3.event.target),event_=event.of(target,arguments),g=d3.select(target),resizing=eventTarget.datum(),resizingX=!/^(n|s)$/.test(resizing)&&x,resizingY=!/^(e|w)$/.test(resizing)&&y,dragging=eventTarget.classed("extent"),dragRestore=d3_event_dragSuppress(),center,origin=d3.mouse(target),offset;var w=d3.select(d3_window).on("keydown.brush",keydown).on("keyup.brush",keyup);if(d3.event.changedTouches){w.on("touchmove.brush",brushmove).on("touchend.brush",brushend)}else{w.on("mousemove.brush",brushmove).on("mouseup.brush",brushend)}g.interrupt().selectAll("*").interrupt();if(dragging){origin[0]=xExtent[0]-origin[0];origin[1]=yExtent[0]-origin[1]}else if(resizing){var ex=+/w$/.test(resizing),ey=+/^n/.test(resizing);offset=[xExtent[1-ex]-origin[0],yExtent[1-ey]-origin[1]];origin[0]=xExtent[ex];origin[1]=yExtent[ey]}else if(d3.event.altKey)center=origin.slice();g.style("pointer-events","none").selectAll(".resize").style("display",null);d3.select("body").style("cursor",eventTarget.style("cursor"));event_({type:"brushstart"});brushmove();function keydown(){if(d3.event.keyCode==32){if(!dragging){center=null;origin[0]-=xExtent[1];origin[1]-=yExtent[1];dragging=2}d3_eventPreventDefault()}}function keyup(){if(d3.event.keyCode==32&&dragging==2){origin[0]+=xExtent[1];origin[1]+=yExtent[1];dragging=0;d3_eventPreventDefault()}}function brushmove(){var point=d3.mouse(target),moved=false;if(offset){point[0]+=offset[0];point[1]+=offset[1]}if(!dragging){if(d3.event.altKey){if(!center)center=[(xExtent[0]+xExtent[1])/2,(yExtent[0]+yExtent[1])/2];origin[0]=xExtent[+(point[0]<center[0])];origin[1]=yExtent[+(point[1]<center[1])]}else center=null}if(resizingX&&move1(point,x,0)){redrawX(g);moved=true}if(resizingY&&move1(point,y,1)){redrawY(g);moved=true}if(moved){redraw(g);event_({type:"brush",mode:dragging?"move":"resize"})}}function move1(point,scale,i){var range=d3_scaleRange(scale),r0=range[0],r1=range[1],position=origin[i],extent=i?yExtent:xExtent,size=extent[1]-extent[0],min,max;if(dragging){r0-=position;r1-=size+position}min=(i?yClamp:xClamp)?Math.max(r0,Math.min(r1,point[i])):point[i];if(dragging){max=(min+=position)+size}else{if(center)position=Math.max(r0,Math.min(r1,2*center[i]-min));if(position<min){max=min;min=position}else{max=position}}if(extent[0]!=min||extent[1]!=max){if(i)yExtentDomain=null;else xExtentDomain=null;extent[0]=min;extent[1]=max;return true}}function brushend(){brushmove();g.style("pointer-events","all").selectAll(".resize").style("display",brush.empty()?"none":null);d3.select("body").style("cursor",null);w.on("mousemove.brush",null).on("mouseup.brush",null).on("touchmove.brush",null).on("touchend.brush",null).on("keydown.brush",null).on("keyup.brush",null);dragRestore();event_({type:"brushend"})}}brush.x=function(z){if(!arguments.length)return x;x=z;resizes=d3_svg_brushResizes[!x<<1|!y];return brush};brush.y=function(z){if(!arguments.length)return y;y=z;resizes=d3_svg_brushResizes[!x<<1|!y];return brush};brush.clamp=function(z){if(!arguments.length)return x&&y?[xClamp,yClamp]:x?xClamp:y?yClamp:null;if(x&&y)xClamp=!!z[0],yClamp=!!z[1];else if(x)xClamp=!!z;else if(y)yClamp=!!z;return brush};brush.extent=function(z){var x0,x1,y0,y1,t;if(!arguments.length){if(x){if(xExtentDomain){x0=xExtentDomain[0],x1=xExtentDomain[1]}else{x0=xExtent[0],x1=xExtent[1];if(x.invert)x0=x.invert(x0),x1=x.invert(x1);if(x1<x0)t=x0,x0=x1,x1=t}}if(y){if(yExtentDomain){y0=yExtentDomain[0],y1=yExtentDomain[1]}else{y0=yExtent[0],y1=yExtent[1];if(y.invert)y0=y.invert(y0),y1=y.invert(y1);if(y1<y0)t=y0,y0=y1,y1=t}}return x&&y?[[x0,y0],[x1,y1]]:x?[x0,x1]:y&&[y0,y1]}if(x){x0=z[0],x1=z[1];if(y)x0=x0[0],x1=x1[0];xExtentDomain=[x0,x1];if(x.invert)x0=x(x0),x1=x(x1);if(x1<x0)t=x0,x0=x1,x1=t;if(x0!=xExtent[0]||x1!=xExtent[1])xExtent=[x0,x1]}if(y){y0=z[0],y1=z[1];if(x)y0=y0[1],y1=y1[1];yExtentDomain=[y0,y1];if(y.invert)y0=y(y0),y1=y(y1);if(y1<y0)t=y0,y0=y1,y1=t;if(y0!=yExtent[0]||y1!=yExtent[1])yExtent=[y0,y1]}return brush};brush.clear=function(){if(!brush.empty()){xExtent=[0,0],yExtent=[0,0];xExtentDomain=yExtentDomain=null}return brush};brush.empty=function(){return!!x&&xExtent[0]==xExtent[1]||!!y&&yExtent[0]==yExtent[1]};return d3.rebind(brush,event,"on")};var d3_svg_brushCursor={n:"ns-resize",e:"ew-resize",s:"ns-resize",w:"ew-resize",nw:"nwse-resize",ne:"nesw-resize",se:"nwse-resize",sw:"nesw-resize"};var d3_svg_brushResizes=[["n","e","s","w","nw","ne","se","sw"],["e","w"],["n","s"],[]];var d3_time=d3.time={},d3_date=Date,d3_time_daySymbols=["Sunday","Monday","Tuesday","Wednesday","Thursday","Friday","Saturday"];function d3_date_utc(){this._=new Date(arguments.length>1?Date.UTC.apply(this,arguments):arguments[0])}d3_date_utc.prototype={getDate:function(){return this._.getUTCDate()},getDay:function(){return this._.getUTCDay()},getFullYear:function(){return this._.getUTCFullYear()},getHours:function(){return this._.getUTCHours()},getMilliseconds:function(){return this._.getUTCMilliseconds()},getMinutes:function(){return this._.getUTCMinutes()},getMonth:function(){return this._.getUTCMonth()},getSeconds:function(){return this._.getUTCSeconds()},getTime:function(){return this._.getTime()},getTimezoneOffset:function(){return 0},valueOf:function(){return this._.valueOf()},setDate:function(){d3_time_prototype.setUTCDate.apply(this._,arguments)},setDay:function(){d3_time_prototype.setUTCDay.apply(this._,arguments)},setFullYear:function(){d3_time_prototype.setUTCFullYear.apply(this._,arguments)},setHours:function(){d3_time_prototype.setUTCHours.apply(this._,arguments)},setMilliseconds:function(){d3_time_prototype.setUTCMilliseconds.apply(this._,arguments)},setMinutes:function(){d3_time_prototype.setUTCMinutes.apply(this._,arguments)},setMonth:function(){d3_time_prototype.setUTCMonth.apply(this._,arguments)},setSeconds:function(){d3_time_prototype.setUTCSeconds.apply(this._,arguments)},setTime:function(){d3_time_prototype.setTime.apply(this._,arguments)}};var d3_time_prototype=Date.prototype;var d3_time_formatDateTime="%a %b %e %X %Y",d3_time_formatDate="%m/%d/%Y",d3_time_formatTime="%H:%M:%S";var d3_time_days=["Sunday","Monday","Tuesday","Wednesday","Thursday","Friday","Saturday"],d3_time_dayAbbreviations=["Sun","Mon","Tue","Wed","Thu","Fri","Sat"],d3_time_months=["January","February","March","April","May","June","July","August","September","October","November","December"],d3_time_monthAbbreviations=["Jan","Feb","Mar","Apr","May","Jun","Jul","Aug","Sep","Oct","Nov","Dec"];function d3_time_interval(local,step,number){function round(date){var d0=local(date),d1=offset(d0,1);return date-d0<d1-date?d0:d1}function ceil(date){step(date=local(new d3_date(date-1)),1);return date}function offset(date,k){step(date=new d3_date(+date),k);return date}function range(t0,t1,dt){var time=ceil(t0),times=[];if(dt>1){while(time<t1){if(!(number(time)%dt))times.push(new Date(+time));step(time,1)}}else{while(time<t1)times.push(new Date(+time)),step(time,1)}return times}function range_utc(t0,t1,dt){try{d3_date=d3_date_utc;var utc=new d3_date_utc;utc._=t0;return range(utc,t1,dt)}finally{d3_date=Date}}local.floor=local;local.round=round;local.ceil=ceil;local.offset=offset;local.range=range;var utc=local.utc=d3_time_interval_utc(local);utc.floor=utc;utc.round=d3_time_interval_utc(round);utc.ceil=d3_time_interval_utc(ceil);utc.offset=d3_time_interval_utc(offset);utc.range=range_utc;return local}function d3_time_interval_utc(method){return function(date,k){try{d3_date=d3_date_utc;var utc=new d3_date_utc;utc._=date;return method(utc,k)._}finally{d3_date=Date}}}d3_time.year=d3_time_interval(function(date){date=d3_time.day(date);date.setMonth(0,1);return date},function(date,offset){date.setFullYear(date.getFullYear()+offset)},function(date){return date.getFullYear()});d3_time.years=d3_time.year.range;d3_time.years.utc=d3_time.year.utc.range;d3_time.day=d3_time_interval(function(date){var day=new d3_date(2e3,0);day.setFullYear(date.getFullYear(),date.getMonth(),date.getDate());return day},function(date,offset){date.setDate(date.getDate()+offset)},function(date){return date.getDate()-1});d3_time.days=d3_time.day.range;d3_time.days.utc=d3_time.day.utc.range;d3_time.dayOfYear=function(date){var year=d3_time.year(date);return Math.floor((date-year-(date.getTimezoneOffset()-year.getTimezoneOffset())*6e4)/864e5)};d3_time_daySymbols.forEach(function(day,i){day=day.toLowerCase();i=7-i;var interval=d3_time[day]=d3_time_interval(function(date){(date=d3_time.day(date)).setDate(date.getDate()-(date.getDay()+i)%7);return date},function(date,offset){date.setDate(date.getDate()+Math.floor(offset)*7)},function(date){var day=d3_time.year(date).getDay();return Math.floor((d3_time.dayOfYear(date)+(day+i)%7)/7)-(day!==i)});d3_time[day+"s"]=interval.range;d3_time[day+"s"].utc=interval.utc.range;d3_time[day+"OfYear"]=function(date){var day=d3_time.year(date).getDay();return Math.floor((d3_time.dayOfYear(date)+(day+i)%7)/7)}});d3_time.week=d3_time.sunday;d3_time.weeks=d3_time.sunday.range;d3_time.weeks.utc=d3_time.sunday.utc.range;d3_time.weekOfYear=d3_time.sundayOfYear;d3_time.format=d3_time_format;function d3_time_format(template){var n=template.length;function format(date){var string=[],i=-1,j=0,c,p,f;while(++i<n){if(template.charCodeAt(i)===37){string.push(template.substring(j,i));if((p=d3_time_formatPads[c=template.charAt(++i)])!=null)c=template.charAt(++i);if(f=d3_time_formats[c])c=f(date,p==null?c==="e"?" ":"0":p);string.push(c);j=i+1}}string.push(template.substring(j,i));return string.join("")}format.parse=function(string){var d={y:1900,m:0,d:1,H:0,M:0,S:0,L:0,Z:null},i=d3_time_parse(d,template,string,0);if(i!=string.length)return null;if("p"in d)d.H=d.H%12+d.p*12;var localZ=d.Z!=null&&d3_date!==d3_date_utc,date=new(localZ?d3_date_utc:d3_date);if("j"in d)date.setFullYear(d.y,0,d.j);else if("w"in d&&("W"in d||"U"in d)){date.setFullYear(d.y,0,1);date.setFullYear(d.y,0,"W"in d?(d.w+6)%7+d.W*7-(date.getDay()+5)%7:d.w+d.U*7-(date.getDay()+6)%7)}else date.setFullYear(d.y,d.m,d.d);date.setHours(d.H+Math.floor(d.Z/100),d.M+d.Z%100,d.S,d.L);return localZ?date._:date};format.toString=function(){return template};return format}function d3_time_parse(date,template,string,j){var c,p,t,i=0,n=template.length,m=string.length;while(i<n){if(j>=m)return-1;c=template.charCodeAt(i++);if(c===37){t=template.charAt(i++);p=d3_time_parsers[t in d3_time_formatPads?template.charAt(i++):t];if(!p||(j=p(date,string,j))<0)return-1}else if(c!=string.charCodeAt(j++)){return-1}}return j}function d3_time_formatRe(names){return new RegExp("^(?:"+names.map(d3.requote).join("|")+")","i")}function d3_time_formatLookup(names){var map=new d3_Map,i=-1,n=names.length;while(++i<n)map.set(names[i].toLowerCase(),i);return map}function d3_time_formatPad(value,fill,width){var sign=value<0?"-":"",string=(sign?-value:value)+"",length=string.length;return sign+(length<width?new Array(width-length+1).join(fill)+string:string)}var d3_time_dayRe=d3_time_formatRe(d3_time_days),d3_time_dayLookup=d3_time_formatLookup(d3_time_days),d3_time_dayAbbrevRe=d3_time_formatRe(d3_time_dayAbbreviations),d3_time_dayAbbrevLookup=d3_time_formatLookup(d3_time_dayAbbreviations),d3_time_monthRe=d3_time_formatRe(d3_time_months),d3_time_monthLookup=d3_time_formatLookup(d3_time_months),d3_time_monthAbbrevRe=d3_time_formatRe(d3_time_monthAbbreviations),d3_time_monthAbbrevLookup=d3_time_formatLookup(d3_time_monthAbbreviations),d3_time_percentRe=/^%/;var d3_time_formatPads={"-":"",_:" ",0:"0"};var d3_time_formats={a:function(d){return d3_time_dayAbbreviations[d.getDay()]},A:function(d){return d3_time_days[d.getDay()]},b:function(d){return d3_time_monthAbbreviations[d.getMonth()]},B:function(d){return d3_time_months[d.getMonth()]},c:d3_time_format(d3_time_formatDateTime),d:function(d,p){return d3_time_formatPad(d.getDate(),p,2)},e:function(d,p){return d3_time_formatPad(d.getDate(),p,2)},H:function(d,p){return d3_time_formatPad(d.getHours(),p,2)},I:function(d,p){return d3_time_formatPad(d.getHours()%12||12,p,2)},j:function(d,p){return d3_time_formatPad(1+d3_time.dayOfYear(d),p,3)},L:function(d,p){return d3_time_formatPad(d.getMilliseconds(),p,3)},m:function(d,p){return d3_time_formatPad(d.getMonth()+1,p,2)},M:function(d,p){return d3_time_formatPad(d.getMinutes(),p,2)},p:function(d){return d.getHours()>=12?"PM":"AM"},S:function(d,p){return d3_time_formatPad(d.getSeconds(),p,2)},U:function(d,p){return d3_time_formatPad(d3_time.sundayOfYear(d),p,2)},w:function(d){return d.getDay()},W:function(d,p){return d3_time_formatPad(d3_time.mondayOfYear(d),p,2)},x:d3_time_format(d3_time_formatDate),X:d3_time_format(d3_time_formatTime),y:function(d,p){return d3_time_formatPad(d.getFullYear()%100,p,2)},Y:function(d,p){return d3_time_formatPad(d.getFullYear()%1e4,p,4)},Z:d3_time_zone,"%":function(){return"%"}};var d3_time_parsers={a:d3_time_parseWeekdayAbbrev,A:d3_time_parseWeekday,b:d3_time_parseMonthAbbrev,B:d3_time_parseMonth,c:d3_time_parseLocaleFull,d:d3_time_parseDay,e:d3_time_parseDay,H:d3_time_parseHour24,I:d3_time_parseHour24,j:d3_time_parseDayOfYear,L:d3_time_parseMilliseconds,m:d3_time_parseMonthNumber,M:d3_time_parseMinutes,p:d3_time_parseAmPm,S:d3_time_parseSeconds,U:d3_time_parseWeekNumberSunday,w:d3_time_parseWeekdayNumber,W:d3_time_parseWeekNumberMonday,x:d3_time_parseLocaleDate,X:d3_time_parseLocaleTime,y:d3_time_parseYear,Y:d3_time_parseFullYear,Z:d3_time_parseZone,"%":d3_time_parseLiteralPercent};function d3_time_parseWeekdayAbbrev(date,string,i){d3_time_dayAbbrevRe.lastIndex=0;var n=d3_time_dayAbbrevRe.exec(string.substring(i));return n?(date.w=d3_time_dayAbbrevLookup.get(n[0].toLowerCase()),i+n[0].length):-1}function d3_time_parseWeekday(date,string,i){d3_time_dayRe.lastIndex=0;var n=d3_time_dayRe.exec(string.substring(i));return n?(date.w=d3_time_dayLookup.get(n[0].toLowerCase()),i+n[0].length):-1}function d3_time_parseWeekdayNumber(date,string,i){d3_time_numberRe.lastIndex=0;var n=d3_time_numberRe.exec(string.substring(i,i+1));return n?(date.w=+n[0],i+n[0].length):-1}function d3_time_parseWeekNumberSunday(date,string,i){d3_time_numberRe.lastIndex=0;var n=d3_time_numberRe.exec(string.substring(i));return n?(date.U=+n[0],i+n[0].length):-1}function d3_time_parseWeekNumberMonday(date,string,i){d3_time_numberRe.lastIndex=0;var n=d3_time_numberRe.exec(string.substring(i));return n?(date.W=+n[0],i+n[0].length):-1}function d3_time_parseMonthAbbrev(date,string,i){d3_time_monthAbbrevRe.lastIndex=0;var n=d3_time_monthAbbrevRe.exec(string.substring(i));return n?(date.m=d3_time_monthAbbrevLookup.get(n[0].toLowerCase()),i+n[0].length):-1}function d3_time_parseMonth(date,string,i){d3_time_monthRe.lastIndex=0;var n=d3_time_monthRe.exec(string.substring(i));return n?(date.m=d3_time_monthLookup.get(n[0].toLowerCase()),i+n[0].length):-1}function d3_time_parseLocaleFull(date,string,i){return d3_time_parse(date,d3_time_formats.c.toString(),string,i)}function d3_time_parseLocaleDate(date,string,i){return d3_time_parse(date,d3_time_formats.x.toString(),string,i)}function d3_time_parseLocaleTime(date,string,i){return d3_time_parse(date,d3_time_formats.X.toString(),string,i)}function d3_time_parseFullYear(date,string,i){d3_time_numberRe.lastIndex=0;var n=d3_time_numberRe.exec(string.substring(i,i+4));return n?(date.y=+n[0],i+n[0].length):-1}function d3_time_parseYear(date,string,i){d3_time_numberRe.lastIndex=0;var n=d3_time_numberRe.exec(string.substring(i,i+2));return n?(date.y=d3_time_expandYear(+n[0]),i+n[0].length):-1}function d3_time_parseZone(date,string,i){return/^[+-]\d{4}$/.test(string=string.substring(i,i+5))?(date.Z=+string,i+5):-1}function d3_time_expandYear(d){return d+(d>68?1900:2e3)}function d3_time_parseMonthNumber(date,string,i){d3_time_numberRe.lastIndex=0;var n=d3_time_numberRe.exec(string.substring(i,i+2));return n?(date.m=n[0]-1,i+n[0].length):-1}function d3_time_parseDay(date,string,i){d3_time_numberRe.lastIndex=0;var n=d3_time_numberRe.exec(string.substring(i,i+2));return n?(date.d=+n[0],i+n[0].length):-1}function d3_time_parseDayOfYear(date,string,i){d3_time_numberRe.lastIndex=0;var n=d3_time_numberRe.exec(string.substring(i,i+3));return n?(date.j=+n[0],i+n[0].length):-1}function d3_time_parseHour24(date,string,i){d3_time_numberRe.lastIndex=0;var n=d3_time_numberRe.exec(string.substring(i,i+2));return n?(date.H=+n[0],i+n[0].length):-1}function d3_time_parseMinutes(date,string,i){d3_time_numberRe.lastIndex=0;var n=d3_time_numberRe.exec(string.substring(i,i+2));return n?(date.M=+n[0],i+n[0].length):-1}function d3_time_parseSeconds(date,string,i){d3_time_numberRe.lastIndex=0;var n=d3_time_numberRe.exec(string.substring(i,i+2));return n?(date.S=+n[0],i+n[0].length):-1}function d3_time_parseMilliseconds(date,string,i){d3_time_numberRe.lastIndex=0;var n=d3_time_numberRe.exec(string.substring(i,i+3));return n?(date.L=+n[0],i+n[0].length):-1}var d3_time_numberRe=/^\s*\d+/;function d3_time_parseAmPm(date,string,i){var n=d3_time_amPmLookup.get(string.substring(i,i+=2).toLowerCase());return n==null?-1:(date.p=n,i)}var d3_time_amPmLookup=d3.map({am:0,pm:1});function d3_time_zone(d){var z=d.getTimezoneOffset(),zs=z>0?"-":"+",zh=~~(abs(z)/60),zm=abs(z)%60;return zs+d3_time_formatPad(zh,"0",2)+d3_time_formatPad(zm,"0",2)}function d3_time_parseLiteralPercent(date,string,i){d3_time_percentRe.lastIndex=0;var n=d3_time_percentRe.exec(string.substring(i,i+1));return n?i+n[0].length:-1}d3_time_format.utc=d3_time_formatUtc;function d3_time_formatUtc(template){var local=d3_time_format(template);function format(date){try{d3_date=d3_date_utc;var utc=new d3_date;utc._=date;return local(utc)}finally{d3_date=Date}}format.parse=function(string){try{d3_date=d3_date_utc;var date=local.parse(string);return date&&date._}finally{d3_date=Date}};format.toString=local.toString;return format}var d3_time_formatIso=d3_time_formatUtc("%Y-%m-%dT%H:%M:%S.%LZ");d3_time_format.iso=Date.prototype.toISOString&&+new Date("2000-01-01T00:00:00.000Z")?d3_time_formatIsoNative:d3_time_formatIso;function d3_time_formatIsoNative(date){return date.toISOString()}d3_time_formatIsoNative.parse=function(string){var date=new Date(string);return isNaN(date)?null:date};d3_time_formatIsoNative.toString=d3_time_formatIso.toString;d3_time.second=d3_time_interval(function(date){return new d3_date(Math.floor(date/1e3)*1e3)},function(date,offset){date.setTime(date.getTime()+Math.floor(offset)*1e3)},function(date){return date.getSeconds()});d3_time.seconds=d3_time.second.range;d3_time.seconds.utc=d3_time.second.utc.range;d3_time.minute=d3_time_interval(function(date){return new d3_date(Math.floor(date/6e4)*6e4)},function(date,offset){date.setTime(date.getTime()+Math.floor(offset)*6e4)},function(date){return date.getMinutes()});d3_time.minutes=d3_time.minute.range;d3_time.minutes.utc=d3_time.minute.utc.range;d3_time.hour=d3_time_interval(function(date){var timezone=date.getTimezoneOffset()/60;return new d3_date((Math.floor(date/36e5-timezone)+timezone)*36e5)},function(date,offset){date.setTime(date.getTime()+Math.floor(offset)*36e5)},function(date){return date.getHours()});d3_time.hours=d3_time.hour.range;d3_time.hours.utc=d3_time.hour.utc.range;d3_time.month=d3_time_interval(function(date){date=d3_time.day(date);date.setDate(1);return date},function(date,offset){date.setMonth(date.getMonth()+offset)},function(date){return date.getMonth()});d3_time.months=d3_time.month.range;d3_time.months.utc=d3_time.month.utc.range;function d3_time_scale(linear,methods,format){function scale(x){return linear(x)}scale.invert=function(x){return d3_time_scaleDate(linear.invert(x))};scale.domain=function(x){if(!arguments.length)return linear.domain().map(d3_time_scaleDate);linear.domain(x);return scale};function tickMethod(extent,count){var span=extent[1]-extent[0],target=span/count,i=d3.bisect(d3_time_scaleSteps,target);return i==d3_time_scaleSteps.length?[methods.year,d3_scale_linearTickRange(extent.map(function(d){return d/31536e6}),count)[2]]:!i?[d3_time_scaleMilliseconds,d3_scale_linearTickRange(extent,count)[2]]:methods[target/d3_time_scaleSteps[i-1]<d3_time_scaleSteps[i]/target?i-1:i]}scale.nice=function(interval,skip){var domain=scale.domain(),extent=d3_scaleExtent(domain),method=interval==null?tickMethod(extent,10):typeof interval==="number"&&tickMethod(extent,interval);if(method)interval=method[0],skip=method[1];function skipped(date){return!isNaN(date)&&!interval.range(date,d3_time_scaleDate(+date+1),skip).length}return scale.domain(d3_scale_nice(domain,skip>1?{floor:function(date){while(skipped(date=interval.floor(date)))date=d3_time_scaleDate(date-1);return date},ceil:function(date){while(skipped(date=interval.ceil(date)))date=d3_time_scaleDate(+date+1);return date}}:interval))};scale.ticks=function(interval,skip){var extent=d3_scaleExtent(scale.domain()),method=interval==null?tickMethod(extent,10):typeof interval==="number"?tickMethod(extent,interval):!interval.range&&[{range:interval},skip];if(method)interval=method[0],skip=method[1];return interval.range(extent[0],d3_time_scaleDate(+extent[1]+1),skip<1?1:skip)};scale.tickFormat=function(){return format};scale.copy=function(){return d3_time_scale(linear.copy(),methods,format)};return d3_scale_linearRebind(scale,linear)}function d3_time_scaleDate(t){return new Date(t)}function d3_time_scaleFormat(formats){return function(date){var i=formats.length-1,f=formats[i];while(!f[1](date))f=formats[--i];return f[0](date)}}var d3_time_scaleSteps=[1e3,5e3,15e3,3e4,6e4,3e5,9e5,18e5,36e5,108e5,216e5,432e5,864e5,1728e5,6048e5,2592e6,7776e6,31536e6];var d3_time_scaleLocalMethods=[[d3_time.second,1],[d3_time.second,5],[d3_time.second,15],[d3_time.second,30],[d3_time.minute,1],[d3_time.minute,5],[d3_time.minute,15],[d3_time.minute,30],[d3_time.hour,1],[d3_time.hour,3],[d3_time.hour,6],[d3_time.hour,12],[d3_time.day,1],[d3_time.day,2],[d3_time.week,1],[d3_time.month,1],[d3_time.month,3],[d3_time.year,1]];var d3_time_scaleLocalFormats=[[d3_time_format("%Y"),d3_true],[d3_time_format("%B"),function(d){return d.getMonth()}],[d3_time_format("%b %d"),function(d){return d.getDate()!=1}],[d3_time_format("%a %d"),function(d){return d.getDay()&&d.getDate()!=1}],[d3_time_format("%I %p"),function(d){return d.getHours()}],[d3_time_format("%I:%M"),function(d){return d.getMinutes()}],[d3_time_format(":%S"),function(d){return d.getSeconds()}],[d3_time_format(".%L"),function(d){return d.getMilliseconds()}]];var d3_time_scaleLocalFormat=d3_time_scaleFormat(d3_time_scaleLocalFormats);d3_time_scaleLocalMethods.year=d3_time.year;d3_time.scale=function(){return d3_time_scale(d3.scale.linear(),d3_time_scaleLocalMethods,d3_time_scaleLocalFormat)};var d3_time_scaleMilliseconds={range:function(start,stop,step){return d3.range(+start,+stop,step).map(d3_time_scaleDate)},floor:d3_identity,ceil:d3_identity};var d3_time_scaleUTCMethods=d3_time_scaleLocalMethods.map(function(m){return[m[0].utc,m[1]]});var d3_time_scaleUTCFormats=[[d3_time_formatUtc("%Y"),d3_true],[d3_time_formatUtc("%B"),function(d){return d.getUTCMonth()}],[d3_time_formatUtc("%b %d"),function(d){return d.getUTCDate()!=1}],[d3_time_formatUtc("%a %d"),function(d){return d.getUTCDay()&&d.getUTCDate()!=1}],[d3_time_formatUtc("%I %p"),function(d){return d.getUTCHours()}],[d3_time_formatUtc("%I:%M"),function(d){return d.getUTCMinutes()}],[d3_time_formatUtc(":%S"),function(d){return d.getUTCSeconds()}],[d3_time_formatUtc(".%L"),function(d){return d.getUTCMilliseconds()}]];var d3_time_scaleUTCFormat=d3_time_scaleFormat(d3_time_scaleUTCFormats);d3_time_scaleUTCMethods.year=d3_time.year.utc;d3_time.scale.utc=function(){return d3_time_scale(d3.scale.linear(),d3_time_scaleUTCMethods,d3_time_scaleUTCFormat)};d3.text=d3_xhrType(function(request){return request.responseText});d3.json=function(url,callback){return d3_xhr(url,"application/json",d3_json,callback)};function d3_json(request){return JSON.parse(request.responseText)}d3.html=function(url,callback){return d3_xhr(url,"text/html",d3_html,callback)};function d3_html(request){var range=d3_document.createRange();range.selectNode(d3_document.body);return range.createContextualFragment(request.responseText)}d3.xml=d3_xhrType(function(request){return request.responseXML});return d3}()},{}],9:[function(require,module,exports){require("./d3");module.exports=d3;(function(){delete this.d3})()},{"./d3":8}],10:[function(require,module,exports){exports.jsqubits=require("./lib/jsqubits");exports.jsqubitsmath=require("./lib/jsqubitsmath.js")},{"./lib/jsqubits":11,"./lib/jsqubitsmath.js":12}],11:[function(require,module,exports){(function(globals){"use strict";function jsqubits(bitString){return jsqubits.QState.fromBits(bitString)}jsqubits.Complex=function(real,imaginary){validateArgs(arguments,1,2,"Must supply a real, and optionally an imaginary, argument to Complex()");imaginary=imaginary||0;this.real=function(){return real};this.imaginary=function(){return imaginary}};jsqubits.Complex.prototype.add=function(other){validateArgs(arguments,1,1,"Must supply 1 parameter to add()");if(typeof other==="number"){return new jsqubits.Complex(this.real()+other,this.imaginary())}return new jsqubits.Complex(this.real()+other.real(),this.imaginary()+other.imaginary())};jsqubits.Complex.prototype.multiply=function(other){validateArgs(arguments,1,1,"Must supply 1 parameter to multiply()");if(typeof other==="number"){return new jsqubits.Complex(this.real()*other,this.imaginary()*other)}return new jsqubits.Complex(this.real()*other.real()-this.imaginary()*other.imaginary(),this.real()*other.imaginary()+this.imaginary()*other.real())};jsqubits.Complex.prototype.conjugate=function(){return new jsqubits.Complex(this.real(),-this.imaginary())};jsqubits.Complex.prototype.toString=function(){if(this.imaginary()===0)return""+this.real();var imaginaryString;if(this.imaginary()===1){imaginaryString="i"}else if(this.imaginary()===-1){imaginaryString="-i"}else{imaginaryString=this.imaginary()+"i"}if(this.real()===0)return imaginaryString;var sign=this.imaginary()<0?"":"+";return this.real()+sign+imaginaryString};jsqubits.Complex.prototype.format=function(options){var realValue=this.real();var imaginaryValue=this.imaginary();if(options&&options.decimalPlaces!=null){var roundingMagnitude=Math.pow(10,options.decimalPlaces);realValue=Math.round(realValue*roundingMagnitude)/roundingMagnitude;imaginaryValue=Math.round(imaginaryValue*roundingMagnitude)/roundingMagnitude}var objectToFormat=new jsqubits.Complex(realValue,imaginaryValue);var prefix="";if(options&&options.spacedSign){if(objectToFormat.real()>0){prefix=" + "}else if(objectToFormat.real()<0){prefix=" - ";objectToFormat=objectToFormat.negate()}else if(objectToFormat.imaginary()>=0){prefix=" + "}else{prefix=" - ";objectToFormat=objectToFormat.negate()}}return prefix+objectToFormat.toString()};jsqubits.Complex.prototype.negate=function(){return new jsqubits.Complex(-this.real(),-this.imaginary())};jsqubits.Complex.prototype.magnitude=function(){return Math.sqrt(this.real()*this.real()+this.imaginary()*this.imaginary())};jsqubits.Complex.prototype.phase=function(){return Math.atan2(this.imaginary(),this.real())};jsqubits.Complex.prototype.subtract=function(other){validateArgs(arguments,1,1,"Must supply 1 parameter to subtract()");if(typeof other==="number"){return new jsqubits.Complex(this.real()-other,this.imaginary())}return new jsqubits.Complex(this.real()-other.real(),this.imaginary()-other.imaginary())};jsqubits.Complex.prototype.eql=function(other){if(!(other instanceof jsqubits.Complex))return false;return this.real()===other.real()&&this.imaginary()===other.imaginary()};jsqubits.complex=function(real,imaginary){return new Complex(real,imaginary)};jsqubits.real=function(real){return new Complex(real,0)};var Complex=jsqubits.Complex;var complex=jsqubits.complex;var real=jsqubits.real;Complex.ZERO=complex(0,0);Complex.SQRT2=real(Math.SQRT2);Complex.SQRT1_2=real(Math.SQRT1_2);jsqubits.ZERO=Complex.ZERO;jsqubits.ONE=complex(1,0);jsqubits.ALL="ALL";jsqubits.roundToZero=1e-7;jsqubits.Measurement=function(numBits,result,newState){this.numBits=numBits;this.result=result;this.newState=newState};jsqubits.Measurement.prototype.toString=function(){return"{result: "+this.result+", newState: "+this.newState+"}"};jsqubits.Measurement.prototype.asBitString=function(){return padState(this.result.toString(2),this.numBits)};jsqubits.StateWithAmplitude=function(numBits,index,amplitude){this.numBits=numBits;this.index=index;this.amplitude=amplitude};jsqubits.StateWithAmplitude.prototype.asNumber=function(){return parseInt(this.index,10)};jsqubits.StateWithAmplitude.prototype.asBitString=function(){return padState(parseInt(this.index,10).toString(2),this.numBits)};jsqubits.QState=function(numBits,amplitudes){validateArgs(arguments,1,"new QState() must be supplied with number of bits (optionally with amplitudes as well)");amplitudes=amplitudes||[jsqubits.ONE];this.numBits=function(){return numBits};this.amplitude=function(basisState){validateArgs(arguments,1,1,"Must supply an index to amplitude()");var numericIndex=typeof basisState==="string"?parseBitString(basisState).value:basisState;return amplitudes[numericIndex]||Complex.ZERO};this.each=function(callBack){validateArgs(arguments,1,1,"Must supply a callback function to each()");for(var index in amplitudes){if(amplitudes.hasOwnProperty(index)){var returnValue=callBack(new jsqubits.StateWithAmplitude(numBits,index,amplitudes[index]));if(returnValue===false)break}}}};jsqubits.QState.fromBits=function(bitString){validateArgs(arguments,1,1,"Must supply a bit string");
var parsedBitString=parseBitString(bitString);var amplitudes={};amplitudes[parsedBitString.value]=jsqubits.ONE;return new jsqubits.QState(parsedBitString.length,amplitudes)};jsqubits.QState.prototype.multiply=function(amount){if(typeof amount==="number"){amount=real(amount)}var amplitudes={};this.each(function(oldAmplitude){amplitudes[oldAmplitude.index]=oldAmplitude.amplitude.multiply(amount)});return new jsqubits.QState(this.numBits(),amplitudes)};jsqubits.QState.prototype.add=function(otherState){var amplitudes={};this.each(function(stateWithAmplitude){amplitudes[stateWithAmplitude.index]=stateWithAmplitude.amplitude});otherState.each(function(stateWithAmplitude){var existingValue=amplitudes[stateWithAmplitude.index]||Complex.ZERO;amplitudes[stateWithAmplitude.index]=stateWithAmplitude.amplitude.add(existingValue)});return new jsqubits.QState(this.numBits(),amplitudes)};jsqubits.QState.prototype.subtract=function(otherState){return this.add(otherState.multiply(-1))};jsqubits.QState.prototype.tensorProduct=function(otherState){var amplitudes={};this.each(function(basisWithAmplitudeA){otherState.each(function(otherBasisWithAmplitude){var newBasisState=(basisWithAmplitudeA.asNumber()<<otherState.numBits())+otherBasisWithAmplitude.asNumber();var newAmplitude=basisWithAmplitudeA.amplitude.multiply(otherBasisWithAmplitude.amplitude);amplitudes[newBasisState]=newAmplitude})});return new jsqubits.QState(this.numBits()+otherState.numBits(),amplitudes)};jsqubits.QState.prototype.kron=jsqubits.QState.prototype.tensorProduct;jsqubits.QState.prototype.controlledHadamard=function(){return function(controlBits,targetBits){validateArgs(arguments,2,2,"Must supply control and target bits to controlledHadamard()");return this.controlledApplicatinOfqBitOperator(controlBits,targetBits,function(amplitudeOf0,amplitudeOf1){var newAmplitudeOf0=amplitudeOf0.add(amplitudeOf1).multiply(Complex.SQRT1_2);var newAmplitudeOf1=amplitudeOf0.subtract(amplitudeOf1).multiply(Complex.SQRT1_2);return{amplitudeOf0:newAmplitudeOf0,amplitudeOf1:newAmplitudeOf1}})}}();jsqubits.QState.prototype.hadamard=function(targetBits){validateArgs(arguments,1,1,"Must supply target bits to hadamard() as either a single index or a range.");return this.controlledHadamard(null,targetBits)};jsqubits.QState.prototype.controlledXRotation=function(controlBits,targetBits,angle){validateArgs(arguments,3,3,"Must supply control bits, target bits, and an angle, to controlledXRotation()");return this.controlledApplicatinOfqBitOperator(controlBits,targetBits,function(amplitudeOf0,amplitudeOf1){var halfAngle=angle/2;var cos=real(Math.cos(halfAngle));var negative_i_sin=complex(0,-Math.sin(halfAngle));var newAmplitudeOf0=amplitudeOf0.multiply(cos).add(amplitudeOf1.multiply(negative_i_sin));var newAmplitudeOf1=amplitudeOf0.multiply(negative_i_sin).add(amplitudeOf1.multiply(cos));return{amplitudeOf0:newAmplitudeOf0,amplitudeOf1:newAmplitudeOf1}})};jsqubits.QState.prototype.rotateX=function(targetBits,angle){validateArgs(arguments,2,2,"Must supply target bits and angle to rotateX.");return this.controlledXRotation(null,targetBits,angle)};jsqubits.QState.prototype.controlledYRotation=function(controlBits,targetBits,angle){validateArgs(arguments,3,3,"Must supply control bits, target bits, and an angle, to controlledYRotation()");return this.controlledApplicatinOfqBitOperator(controlBits,targetBits,function(amplitudeOf0,amplitudeOf1){var halfAngle=angle/2;var cos=real(Math.cos(halfAngle));var sin=real(Math.sin(halfAngle));var newAmplitudeOf0=amplitudeOf0.multiply(cos).add(amplitudeOf1.multiply(sin.negate()));var newAmplitudeOf1=amplitudeOf0.multiply(sin).add(amplitudeOf1.multiply(cos));return{amplitudeOf0:newAmplitudeOf0,amplitudeOf1:newAmplitudeOf1}})};jsqubits.QState.prototype.rotateY=function(targetBits,angle){validateArgs(arguments,2,2,"Must supply target bits and angle to rotateY.");return this.controlledYRotation(null,targetBits,angle)};jsqubits.QState.prototype.controlledZRotation=function(controlBits,targetBits,angle){validateArgs(arguments,3,3,"Must supply control bits, target bits, and an angle to controlledZRotation()");return this.controlledApplicatinOfqBitOperator(controlBits,targetBits,function(amplitudeOf0,amplitudeOf1){var halfAngle=angle/2;var cos=real(Math.cos(halfAngle));var i_sin=complex(0,Math.sin(halfAngle));var newAmplitudeOf0=amplitudeOf0.multiply(cos.subtract(i_sin));var newAmplitudeOf1=amplitudeOf1.multiply(cos.add(i_sin));return{amplitudeOf0:newAmplitudeOf0,amplitudeOf1:newAmplitudeOf1}})};jsqubits.QState.prototype.rotateZ=function(targetBits,angle){validateArgs(arguments,2,2,"Must supply target bits and angle to rotateZ.");return this.controlledZRotation(null,targetBits,angle)};jsqubits.QState.prototype.controlledR=function(controlBits,targetBits,angle){validateArgs(arguments,3,3,"Must supply control and target bits, and an angle to controlledR().");return this.controlledApplicatinOfqBitOperator(controlBits,targetBits,function(amplitudeOf0,amplitudeOf1){var cos=real(Math.cos(angle));var i_sin=complex(0,Math.sin(angle));var newAmplitudeOf0=amplitudeOf0;var newAmplitudeOf1=amplitudeOf1.multiply(cos.add(i_sin));return{amplitudeOf0:newAmplitudeOf0,amplitudeOf1:newAmplitudeOf1}})};jsqubits.QState.prototype.r=function(targetBits,angle){validateArgs(arguments,2,2,"Must supply target bits and angle to r().");return this.controlledR(null,targetBits,angle)};jsqubits.QState.prototype.R=jsqubits.QState.prototype.r;jsqubits.QState.prototype.controlledX=function(controlBits,targetBits){validateArgs(arguments,2,2,"Must supply control and target bits to cnot() / controlledX().");return this.controlledApplicatinOfqBitOperator(controlBits,targetBits,function(amplitudeOf0,amplitudeOf1){return{amplitudeOf0:amplitudeOf1,amplitudeOf1:amplitudeOf0}})};jsqubits.QState.prototype.cnot=jsqubits.QState.prototype.controlledX;jsqubits.QState.prototype.x=function(targetBits){validateArgs(arguments,1,1,"Must supply target bits to x() / not().");return this.controlledX(null,targetBits)};jsqubits.QState.prototype.not=jsqubits.QState.prototype.x;jsqubits.QState.prototype.X=jsqubits.QState.prototype.x;jsqubits.QState.prototype.controlledY=function(controlBits,targetBits){validateArgs(arguments,2,2,"Must supply control and target bits to controlledY().");return this.controlledApplicatinOfqBitOperator(controlBits,targetBits,function(amplitudeOf0,amplitudeOf1){return{amplitudeOf0:amplitudeOf1.multiply(complex(0,-1)),amplitudeOf1:amplitudeOf0.multiply(complex(0,1))}})};jsqubits.QState.prototype.y=function(targetBits){validateArgs(arguments,1,1,"Must supply target bits to y().");return this.controlledY(null,targetBits)};jsqubits.QState.prototype.Y=jsqubits.QState.prototype.y;jsqubits.QState.prototype.controlledZ=function(controlBits,targetBits){validateArgs(arguments,2,2,"Must supply control and target bits to controlledZ().");return this.controlledApplicatinOfqBitOperator(controlBits,targetBits,function(amplitudeOf0,amplitudeOf1){return{amplitudeOf0:amplitudeOf0,amplitudeOf1:amplitudeOf1.negate()}})};jsqubits.QState.prototype.z=function(targetBits){validateArgs(arguments,1,1,"Must supply target bits to z().");return this.controlledZ(null,targetBits)};jsqubits.QState.prototype.Z=jsqubits.QState.prototype.z;jsqubits.QState.prototype.controlledS=function(controlBits,targetBits){validateArgs(arguments,2,2,"Must supply control and target bits to controlledS().");return this.controlledApplicatinOfqBitOperator(controlBits,targetBits,function(amplitudeOf0,amplitudeOf1){return{amplitudeOf0:amplitudeOf0,amplitudeOf1:amplitudeOf1.multiply(complex(0,1))}})};jsqubits.QState.prototype.s=function(targetBits){validateArgs(arguments,1,1,"Must supply target bits to s().");return this.controlledS(null,targetBits)};jsqubits.QState.prototype.S=jsqubits.QState.prototype.s;jsqubits.QState.prototype.controlledT=function(){var expPiOn4=complex(Math.SQRT1_2,Math.SQRT1_2);return function(controlBits,targetBits){validateArgs(arguments,2,2,"Must supply control and target bits to controlledT().");return this.controlledApplicatinOfqBitOperator(controlBits,targetBits,function(amplitudeOf0,amplitudeOf1){return{amplitudeOf0:amplitudeOf0,amplitudeOf1:amplitudeOf1.multiply(expPiOn4)}})}}();jsqubits.QState.prototype.t=function(targetBits){validateArgs(arguments,1,1,"Must supply target bits to t().");return this.controlledT(null,targetBits)};jsqubits.QState.prototype.T=jsqubits.QState.prototype.t;jsqubits.QState.prototype.controlledSwap=function(controlBits,targetBit1,targetBit2){validateArgs(arguments,3,3,"Must supply controlBits, targetBit1, and targetBit2 to controlledSwap()");var newAmplitudes={};if(controlBits!=null){controlBits=convertBitQualifierToBitArray(controlBits,this.numBits())}var controlBitMask=createBitMask(controlBits);var bit1Mask=1<<targetBit1;var bit2Mask=1<<targetBit2;this.each(function(stateWithAmplitude){var state=stateWithAmplitude.asNumber();var newState=state;if(controlBits==null||(state&controlBitMask)===controlBitMask){var newBit2=(state&bit1Mask)>>targetBit1<<targetBit2;var newBit1=(state&bit2Mask)>>targetBit2<<targetBit1;newState=state&~bit1Mask&~bit2Mask|newBit1|newBit2}newAmplitudes[newState]=stateWithAmplitude.amplitude});return new jsqubits.QState(this.numBits(),newAmplitudes)};jsqubits.QState.prototype.swap=function(targetBit1,targetBit2){validateArgs(arguments,2,2,"Must supply targetBit1 and targetBit2 to swap()");return this.controlledSwap(null,targetBit1,targetBit2)};jsqubits.QState.prototype.toffoli=function(){validateArgs(arguments,2,"At least one control bit and a target bit must be supplied to calls to toffoli()");var targetBit=arguments[arguments.length-1];var controlBits=[];for(var i=0;i<arguments.length-1;i++){controlBits.push(arguments[i])}return this.controlledX(controlBits,targetBit)};jsqubits.QState.prototype.controlledApplicatinOfqBitOperator=function(){function applyToOneBit(controlBits,targetBit,qbitFunction,qState){var newAmplitudes={};var statesThatCanBeSkipped={};var targetBitMask=1<<targetBit;var controlBitMask=createBitMask(controlBits);qState.each(function(stateWithAmplitude){var state=stateWithAmplitude.asNumber();if(statesThatCanBeSkipped[stateWithAmplitude.index])return;statesThatCanBeSkipped[state^targetBitMask]=true;var indexOf1=state|targetBitMask;var indexOf0=indexOf1-targetBitMask;if(controlBits==null||(state&controlBitMask)===controlBitMask){var result=qbitFunction(qState.amplitude(indexOf0),qState.amplitude(indexOf1));sparseAssign(newAmplitudes,indexOf0,result.amplitudeOf0);sparseAssign(newAmplitudes,indexOf1,result.amplitudeOf1)}else{sparseAssign(newAmplitudes,indexOf0,qState.amplitude(indexOf0));sparseAssign(newAmplitudes,indexOf1,qState.amplitude(indexOf1))}});return new jsqubits.QState(qState.numBits(),newAmplitudes)}return function(controlBits,targetBits,qbitFunction){validateArgs(arguments,3,3,"Must supply control bits, target bits, and qbitFunction to controlledApplicatinOfqBitOperator().");var targetBitArray=convertBitQualifierToBitArray(targetBits,this.numBits());var controlBitArray=null;if(controlBits!=null){controlBitArray=convertBitQualifierToBitArray(controlBits,this.numBits());validateControlAndTargetBitsDontOverlap(controlBitArray,targetBitArray)}var result=this;for(var i=0;i<targetBitArray.length;i++){var targetBit=targetBitArray[i];result=applyToOneBit(controlBitArray,targetBit,qbitFunction,result)}return result}}();jsqubits.QState.prototype.applyFunction=function(){function validateTargetBitRangesDontOverlap(controlBits,targetBits){if(controlBits.to>=targetBits.from&&targetBits.to>=controlBits.from){throw"control and target bits must not be the same nor overlap"}}return function(inputBits,targetBits,functionToApply){validateArgs(arguments,3,3,"Must supply control bits, target bits, and functionToApply to applyFunction().");var qState=this;var inputBitRange=convertBitQualifierToBitRange(inputBits,this.numBits());var targetBitRange=convertBitQualifierToBitRange(targetBits,this.numBits());validateTargetBitRangesDontOverlap(inputBitRange,targetBitRange);var newAmplitudes={};var statesThatCanBeSkipped={};var highBitMask=(1<<inputBitRange.to+1)-1;var targetBitMask=(1<<1+targetBitRange.to-targetBitRange.from)-1<<targetBitRange.from;this.each(function(stateWithAmplitude){var state=stateWithAmplitude.asNumber();if(statesThatCanBeSkipped[stateWithAmplitude.index])return;var input=(state&highBitMask)>>inputBitRange.from;var result=functionToApply(input)<<targetBitRange.from&targetBitMask;var resultingState=state^result;if(resultingState===state){sparseAssign(newAmplitudes,state,stateWithAmplitude.amplitude)}else{statesThatCanBeSkipped[resultingState]=true;sparseAssign(newAmplitudes,state,qState.amplitude(resultingState));sparseAssign(newAmplitudes,resultingState,stateWithAmplitude.amplitude)}});return new jsqubits.QState(this.numBits(),newAmplitudes)}}();jsqubits.QState.prototype.random=Math.random;jsqubits.QState.prototype.normalize=function(){var amplitudes={};var sumOfMagnitudeSqaures=0;this.each(function(stateWithAmplitude){var magnitude=stateWithAmplitude.amplitude.magnitude();sumOfMagnitudeSqaures+=magnitude*magnitude});var scale=real(1/Math.sqrt(sumOfMagnitudeSqaures));this.each(function(stateWithAmplitude){amplitudes[stateWithAmplitude.index]=stateWithAmplitude.amplitude.multiply(scale)});return new jsqubits.QState(this.numBits(),amplitudes)};jsqubits.QState.prototype.measure=function(bits){validateArgs(arguments,1,1,"Must supply bits to be measured to measure().");var numBits=this.numBits();var bitArray=convertBitQualifierToBitArray(bits,numBits);var chosenState=chooseRandomBasisState(this);var bitMask=createBitMask(bitArray);var maskedChosenState=chosenState&bitMask;var newAmplitudes={};this.each(function(stateWithAmplitude){var state=stateWithAmplitude.asNumber();if((state&bitMask)===maskedChosenState){newAmplitudes[state]=stateWithAmplitude.amplitude}});var measurementOutcome=0;for(var bitIndex=numBits-1;bitIndex>=0;bitIndex--){if(bitArray.indexOf(bitIndex)>=0){measurementOutcome=measurementOutcome<<1;if(chosenState&1<<bitIndex){measurementOutcome+=1}}}var newState=new jsqubits.QState(this.numBits(),newAmplitudes).normalize();return new jsqubits.Measurement(bitArray.length,measurementOutcome,newState)};jsqubits.QState.prototype.qft=function(targetBits){function qft(qstate,targetBits){var bitIndex=targetBits[0];if(targetBits.length>1){qstate=qft(qstate,targetBits.slice(1));for(var index=1;index<targetBits.length;index++){var otherBitIndex=targetBits[index];var angle=2*Math.PI/(1<<index+1);qstate=qstate.controlledR(bitIndex,otherBitIndex,angle)}}return qstate.hadamard(bitIndex)}function reverseBits(qstate,targetBits){while(targetBits.length>1){qstate=qstate.swap(targetBits[0],targetBits[targetBits.length-1]);targetBits=targetBits.slice(1,targetBits.length-1)}return qstate}validateArgs(arguments,1,1,"Must supply bits to be measured to qft().");var targetBitArray=convertBitQualifierToBitArray(targetBits,this.numBits());var newState=qft(this,targetBitArray);return reverseBits(newState,targetBitArray)};jsqubits.QState.prototype.eql=function(other){function lhsAmplitudesHaveMatchingRhsAmplitudes(lhs,rhs){var result=true;lhs.each(function(stateWithAmplitude){if(!stateWithAmplitude.amplitude.eql(rhs.amplitude(stateWithAmplitude.asNumber()))){result=false;return false}});return result}if(!other)return false;if(!(other instanceof jsqubits.QState))return false;if(this.numBits()!==other.numBits())return false;return lhsAmplitudesHaveMatchingRhsAmplitudes(this,other)&&lhsAmplitudesHaveMatchingRhsAmplitudes(other,this)};jsqubits.QState.prototype.toString=function(){function formatAmplitude(amplitude,formatFlags){var amplitudeString=amplitude.format(formatFlags);return amplitudeString==="1"?"":amplitudeString+" "}function compareStatesWithAmplitudes(a,b){return a.asNumber()-b.asNumber()}function sortedNonZeroStates(qState){var nonZeroStates=[];qState.each(function(stateWithAmplitude){nonZeroStates.push(stateWithAmplitude)});nonZeroStates.sort(compareStatesWithAmplitudes);return nonZeroStates}return function(){var result,formatFlags,nonZeroStates,stateWithAmplitude,i;result="";formatFlags={decimalPlaces:4};nonZeroStates=sortedNonZeroStates(this);for(i=0;i<nonZeroStates.length;i++){if(result!=="")formatFlags.spacedSign=true;stateWithAmplitude=nonZeroStates[i];result=result+formatAmplitude(stateWithAmplitude.amplitude,formatFlags)+"|"+stateWithAmplitude.asBitString()+">"}return result}}();function validateArgs(args,minimum){var maximum=1e4;var message="Must supply at least "+minimum+" parameters.";if(arguments.length>4)throw"Internal error: too many arguments to validateArgs";if(arguments.length===4){maximum=arguments[2];message=arguments[3]}else if(arguments.length===3){message=arguments[2]}if(args.length<minimum||args.length>maximum){throw message}}function parseBitString(bitString){bitString=bitString.replace(/^\|/,"").replace(/>$/,"");return{value:parseInt(bitString,2),length:bitString.length}}function sparseAssign(array,index,value){if(value.magnitude()>jsqubits.roundToZero){array[index]=value}}function convertBitQualifierToBitRange(bits,numBits){if(bits==null){throw"bit qualification must be supplied"}else if(bits===jsqubits.ALL){return{from:0,to:numBits-1}}else if(typeof bits==="number"){return{from:bits,to:bits}}else if(bits.from!=null&&bits.to!=null){if(bits.from>bits.to){throw"bit range must have 'from' being less than or equal to 'to'"}return bits}else{throw"bit qualification must be either: a number, jsqubits.ALL, or {from: n, to: m}"}}function validateControlAndTargetBitsDontOverlap(controlBits,targetBits){for(var i=0;i<controlBits.length;i++){var controlBit=controlBits[i];for(var j=0;j<targetBits.length;j++){if(controlBit===targetBits[j]){throw"control and target bits must not be the same nor overlap"}}}}function chooseRandomBasisState(qState){var randomNumber=qState.random();var randomStateString;var accumulativeSquareAmplitudeMagnitude=0;qState.each(function(stateWithAmplitude){var magnitude=stateWithAmplitude.amplitude.magnitude();accumulativeSquareAmplitudeMagnitude+=magnitude*magnitude;randomStateString=stateWithAmplitude.index;if(accumulativeSquareAmplitudeMagnitude>randomNumber){return false}});return parseInt(randomStateString,10)}function bitRangeAsArray(low,high){if(low>high){throw"bit range must have 'from' being less than or equal to 'to'"}var result=[];for(var i=low;i<=high;i++){result.push(i)}return result}function convertBitQualifierToBitArray(bits,numBits){if(bits==null){throw"bit qualification must be supplied"}if(bits instanceof Array){return bits}if(typeof bits==="number"){return[bits]}if(bits===jsqubits.ALL){return bitRangeAsArray(0,numBits-1)}if(bits.from!=null&&bits.to!=null){return bitRangeAsArray(bits.from,bits.to)}throw"bit qualification must be either: a number, an array of numbers, jsqubits.ALL, or {from: n, to: m}"}function padState(state,numBits){var paddingLength=numBits-state.length;for(var i=0;i<paddingLength;i++){state="0"+state}return state}function createBitMask(bits){var mask=null;if(bits){mask=0;for(var i=0;i<bits.length;i++){mask+=1<<bits[i]}}return mask}if(typeof define!=="undefined"&&define.amd){define([],function(){return jsqubits})}else if(typeof module!=="undefined"&&module.exports){module.exports=jsqubits}else{globals.jsqubits=jsqubits}})(this)},{}],12:[function(require,module,exports){(function(globals){"use strict";var jsqubitsmath={};jsqubitsmath.powerMod=function(x,y,m){if(y===0)return 1;if(y===1)return x;var halfY=Math.floor(y/2);var powerHalfY=jsqubitsmath.powerMod(x,halfY,m);var result=powerHalfY*powerHalfY%m;if(y%2===1)result=x*result%m;return result};jsqubitsmath.powerFactor=function(n){var log2n=Math.log(n)/Math.log(2);var y=Math.floor(log2n);if(log2n===y){return 2}y--;for(;y>1;y--){var x=Math.pow(n,1/y);if(approximatelyInteger(x)){return Math.round(x)}}return 0};jsqubitsmath.gcd=function(a,b){while(b!==0){var c=a%b;a=b;b=c}return a};jsqubitsmath.lcm=function(a,b){return a*b/jsqubitsmath.gcd(a,b)};jsqubitsmath.continuedFraction=function(targetValue,precision){var firstValue,remainder;if(Math.abs(targetValue)>=1){firstValue=roundTowardsZero(targetValue);remainder=targetValue-firstValue}else{firstValue=0;remainder=targetValue}var twoAgo={numerator:1,denominator:0};var oneAgo={numerator:firstValue,denominator:1};var quotients=[firstValue];while(Math.abs(targetValue-oneAgo.numerator/oneAgo.denominator)>precision){var reciprocal=1/remainder;var quotient=roundTowardsZero(reciprocal);remainder=reciprocal-quotient;quotients.push(quotient);var current={numerator:quotient*oneAgo.numerator+twoAgo.numerator,denominator:quotient*oneAgo.denominator+twoAgo.denominator};twoAgo=oneAgo;oneAgo=current}var numerator=oneAgo.numerator;var denominator=oneAgo.denominator;if(oneAgo.denominator<0){numerator*=-1;denominator*=-1}return{quotients:quotients,numerator:numerator,denominator:denominator}};jsqubitsmath.findNullSpaceMod2=function(){function attemptToMakePivot(a,colIndex,pivotRowIndex){var colBitMask=1<<colIndex;if(colBitMask&a[pivotRowIndex])return;for(var rowIndex=pivotRowIndex+1;rowIndex<a.length;rowIndex++){if(colBitMask&a[rowIndex]){var tmp=a[pivotRowIndex];a[pivotRowIndex]=a[rowIndex];a[rowIndex]=tmp;return}}}function makeReducedRowEchelonForm(a,width,pivotColumnIndexes){var pivotRowIndex=0;for(var pivotColIndex=width-1;pivotColIndex>=0;pivotColIndex--){attemptToMakePivot(a,pivotColIndex,pivotRowIndex);var colBitMask=1<<pivotColIndex;if(colBitMask&a[pivotRowIndex]){pivotColumnIndexes[pivotRowIndex]=pivotColIndex;zeroOutAboveAndBelow(a,pivotColIndex,pivotRowIndex);pivotRowIndex++}}}function zeroOutAboveAndBelow(a,pivotColIndex,pivotRowIndex){var pivotRow=a[pivotRowIndex];var colBitMask=1<<pivotColIndex;for(var rowIndex=0;rowIndex<a.length;rowIndex++){if(rowIndex!==pivotRowIndex&&colBitMask&a[rowIndex]){a[rowIndex]=a[rowIndex]^pivotRow}}}function specialSolutionForColumn(a,pivotColumnIndexes,colIndex,pivotNumber){var columnMask=1<<colIndex;var specialSolution=columnMask;for(var rowIndex=0;rowIndex<pivotNumber;rowIndex++){if(a[rowIndex]&columnMask){specialSolution+=1<<pivotColumnIndexes[rowIndex]}}return specialSolution}function specialSolutions(a,width,pivotColumnIndexes){var results=[];var pivotNumber=0;var nextPivotColumnIndex=pivotColumnIndexes[pivotNumber];for(var colIndex=width-1;colIndex>=0;colIndex--){if(colIndex===nextPivotColumnIndex){pivotNumber++;nextPivotColumnIndex=pivotColumnIndexes[pivotNumber]}else{results.push(specialSolutionForColumn(a,pivotColumnIndexes,colIndex,pivotNumber))}}return results}return function(a,width){a=cloneArray(a);var pivotColumnIndexes=[];makeReducedRowEchelonForm(a,width,pivotColumnIndexes);return specialSolutions(a,width,pivotColumnIndexes)}}();function roundTowardsZero(value){return value>=0?Math.floor(value):Math.ceil(value)}function approximatelyInteger(x){return Math.abs(x-Math.round(x))<1e-7}function cloneArray(a){var result=[];for(var i=0;i<a.length;i++){result[i]=a[i]}return result}if(typeof define!=="undefined"&&define.amd){define([],function(){return jsqubitsmath})}else if(typeof module!=="undefined"&&module.exports){module.exports=jsqubitsmath}else{globals.jsqubitsmath=jsqubitsmath}})(this)},{}],13:[function(require,module,exports){var global=typeof self!=="undefined"?self:typeof window!=="undefined"?window:{};(function(){var undefined;var arrayPool=[],objectPool=[];var idCounter=0;var keyPrefix=+new Date+"";var largeArraySize=75;var maxPoolSize=40;var whitespace=" 	\f ﻿"+"\n\r\u2028\u2029"+" ᠎             　";var reEmptyStringLeading=/\b__p \+= '';/g,reEmptyStringMiddle=/\b(__p \+=) '' \+/g,reEmptyStringTrailing=/(__e\(.*?\)|\b__t\)) \+\n'';/g;var reEsTemplate=/\$\{([^\\}]*(?:\\.[^\\}]*)*)\}/g;var reFlags=/\w*$/;var reFuncName=/^\s*function[ \n\r\t]+\w/;var reInterpolate=/<%=([\s\S]+?)%>/g;var reLeadingSpacesAndZeros=RegExp("^["+whitespace+"]*0+(?=.$)");var reNoMatch=/($^)/;var reThis=/\bthis\b/;var reUnescapedString=/['\n\r\t\u2028\u2029\\]/g;var contextProps=["Array","Boolean","Date","Function","Math","Number","Object","RegExp","String","_","attachEvent","clearTimeout","isFinite","isNaN","parseInt","setTimeout"];var templateCounter=0;var argsClass="[object Arguments]",arrayClass="[object Array]",boolClass="[object Boolean]",dateClass="[object Date]",funcClass="[object Function]",numberClass="[object Number]",objectClass="[object Object]",regexpClass="[object RegExp]",stringClass="[object String]";var cloneableClasses={};cloneableClasses[funcClass]=false;cloneableClasses[argsClass]=cloneableClasses[arrayClass]=cloneableClasses[boolClass]=cloneableClasses[dateClass]=cloneableClasses[numberClass]=cloneableClasses[objectClass]=cloneableClasses[regexpClass]=cloneableClasses[stringClass]=true;var debounceOptions={leading:false,maxWait:0,trailing:false};var descriptor={configurable:false,enumerable:false,value:null,writable:false};var objectTypes={"boolean":false,"function":true,object:true,number:false,string:false,undefined:false};var stringEscapes={"\\":"\\","'":"'","\n":"n","\r":"r","	":"t","\u2028":"u2028","\u2029":"u2029"};var root=objectTypes[typeof window]&&window||this;var freeExports=objectTypes[typeof exports]&&exports&&!exports.nodeType&&exports;var freeModule=objectTypes[typeof module]&&module&&!module.nodeType&&module;var moduleExports=freeModule&&freeModule.exports===freeExports&&freeExports;var freeGlobal=objectTypes[typeof global]&&global;if(freeGlobal&&(freeGlobal.global===freeGlobal||freeGlobal.window===freeGlobal)){root=freeGlobal}function baseIndexOf(array,value,fromIndex){var index=(fromIndex||0)-1,length=array?array.length:0;while(++index<length){if(array[index]===value){return index}}return-1}function cacheIndexOf(cache,value){var type=typeof value;cache=cache.cache;if(type=="boolean"||value==null){return cache[value]?0:-1}if(type!="number"&&type!="string"){type="object"}var key=type=="number"?value:keyPrefix+value;cache=(cache=cache[type])&&cache[key];return type=="object"?cache&&baseIndexOf(cache,value)>-1?0:-1:cache?0:-1}function cachePush(value){var cache=this.cache,type=typeof value;if(type=="boolean"||value==null){cache[value]=true}else{if(type!="number"&&type!="string"){type="object"}var key=type=="number"?value:keyPrefix+value,typeCache=cache[type]||(cache[type]={});if(type=="object"){(typeCache[key]||(typeCache[key]=[])).push(value)}else{typeCache[key]=true}}}function charAtCallback(value){return value.charCodeAt(0)}function compareAscending(a,b){var ac=a.criteria,bc=b.criteria,index=-1,length=ac.length;while(++index<length){var value=ac[index],other=bc[index];if(value!==other){if(value>other||typeof value=="undefined"){return 1}if(value<other||typeof other=="undefined"){return-1}}}return a.index-b.index}function createCache(array){var index=-1,length=array.length,first=array[0],mid=array[length/2|0],last=array[length-1];if(first&&typeof first=="object"&&mid&&typeof mid=="object"&&last&&typeof last=="object"){return false}var cache=getObject();cache["false"]=cache["null"]=cache["true"]=cache["undefined"]=false;var result=getObject();result.array=array;result.cache=cache;result.push=cachePush;while(++index<length){result.push(array[index])}return result}function escapeStringChar(match){return"\\"+stringEscapes[match]}function getArray(){return arrayPool.pop()||[]}function getObject(){return objectPool.pop()||{array:null,cache:null,criteria:null,"false":false,index:0,"null":false,number:null,object:null,push:null,string:null,"true":false,undefined:false,value:null}}function releaseArray(array){array.length=0;if(arrayPool.length<maxPoolSize){arrayPool.push(array)}}function releaseObject(object){var cache=object.cache;if(cache){releaseObject(cache)}object.array=object.cache=object.criteria=object.object=object.number=object.string=object.value=null;if(objectPool.length<maxPoolSize){objectPool.push(object)}}function slice(array,start,end){start||(start=0);if(typeof end=="undefined"){end=array?array.length:0}var index=-1,length=end-start||0,result=Array(length<0?0:length);while(++index<length){result[index]=array[start+index]}return result}function runInContext(context){context=context?_.defaults(root.Object(),context,_.pick(root,contextProps)):root;var Array=context.Array,Boolean=context.Boolean,Date=context.Date,Function=context.Function,Math=context.Math,Number=context.Number,Object=context.Object,RegExp=context.RegExp,String=context.String,TypeError=context.TypeError;var arrayRef=[];var objectProto=Object.prototype;var oldDash=context._;var toString=objectProto.toString;var reNative=RegExp("^"+String(toString).replace(/[.*+?^${}()|[\]\\]/g,"\\$&").replace(/toString| for [^\]]+/g,".*?")+"$");var ceil=Math.ceil,clearTimeout=context.clearTimeout,floor=Math.floor,fnToString=Function.prototype.toString,getPrototypeOf=isNative(getPrototypeOf=Object.getPrototypeOf)&&getPrototypeOf,hasOwnProperty=objectProto.hasOwnProperty,push=arrayRef.push,setTimeout=context.setTimeout,splice=arrayRef.splice,unshift=arrayRef.unshift;var defineProperty=function(){try{var o={},func=isNative(func=Object.defineProperty)&&func,result=func(o,o,o)&&func}catch(e){}return result}();var nativeCreate=isNative(nativeCreate=Object.create)&&nativeCreate,nativeIsArray=isNative(nativeIsArray=Array.isArray)&&nativeIsArray,nativeIsFinite=context.isFinite,nativeIsNaN=context.isNaN,nativeKeys=isNative(nativeKeys=Object.keys)&&nativeKeys,nativeMax=Math.max,nativeMin=Math.min,nativeParseInt=context.parseInt,nativeRandom=Math.random;var ctorByClass={};ctorByClass[arrayClass]=Array;ctorByClass[boolClass]=Boolean;ctorByClass[dateClass]=Date;ctorByClass[funcClass]=Function;ctorByClass[objectClass]=Object;ctorByClass[numberClass]=Number;ctorByClass[regexpClass]=RegExp;ctorByClass[stringClass]=String;function lodash(value){return value&&typeof value=="object"&&!isArray(value)&&hasOwnProperty.call(value,"__wrapped__")?value:new lodashWrapper(value)}function lodashWrapper(value,chainAll){this.__chain__=!!chainAll;this.__wrapped__=value}lodashWrapper.prototype=lodash.prototype;var support=lodash.support={};support.funcDecomp=!isNative(context.WinRTError)&&reThis.test(runInContext);support.funcNames=typeof Function.name=="string";lodash.templateSettings={escape:/<%-([\s\S]+?)%>/g,evaluate:/<%([\s\S]+?)%>/g,interpolate:reInterpolate,variable:"",imports:{_:lodash}};function baseBind(bindData){var func=bindData[0],partialArgs=bindData[2],thisArg=bindData[4];function bound(){if(partialArgs){var args=slice(partialArgs);push.apply(args,arguments)}if(this instanceof bound){var thisBinding=baseCreate(func.prototype),result=func.apply(thisBinding,args||arguments);return isObject(result)?result:thisBinding}return func.apply(thisArg,args||arguments)}setBindData(bound,bindData);return bound}function baseClone(value,isDeep,callback,stackA,stackB){if(callback){var result=callback(value);if(typeof result!="undefined"){return result}}var isObj=isObject(value);if(isObj){var className=toString.call(value);if(!cloneableClasses[className]){return value}var ctor=ctorByClass[className];switch(className){case boolClass:case dateClass:return new ctor(+value);case numberClass:case stringClass:return new ctor(value);case regexpClass:result=ctor(value.source,reFlags.exec(value));result.lastIndex=value.lastIndex;return result}}else{return value}var isArr=isArray(value);if(isDeep){var initedStack=!stackA;stackA||(stackA=getArray());stackB||(stackB=getArray());var length=stackA.length;while(length--){if(stackA[length]==value){return stackB[length]}}result=isArr?ctor(value.length):{}}else{result=isArr?slice(value):assign({},value)}if(isArr){if(hasOwnProperty.call(value,"index")){result.index=value.index}if(hasOwnProperty.call(value,"input")){result.input=value.input}}if(!isDeep){return result}stackA.push(value);stackB.push(result);(isArr?forEach:forOwn)(value,function(objValue,key){result[key]=baseClone(objValue,isDeep,callback,stackA,stackB)});if(initedStack){releaseArray(stackA);releaseArray(stackB)}return result}function baseCreate(prototype,properties){return isObject(prototype)?nativeCreate(prototype):{}
